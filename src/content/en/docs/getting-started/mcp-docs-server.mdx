---
title: "Using with Cursor/Windsurf | Getting Started | Kastrax Docs"
description: "Learn how to use the Kastrax MCP documentation server in your IDE to turn it into an agentic Kastrax expert."
---
...
`@kastrax/mcp-docs-server` provides direct access to Kastrax's complete knowledge base in Cursor, Windsurf, Cline, or any other IDE that supports MCP.
...
- "What's the latest Kastrax news around MCP?"
- "Does Kastrax support `[provider]` speech and voice APIs? Show me an example in my code of how I can use it."
...
- "How does working memory behave in Kastrax and how can I use it to do `[task]`? It doesn't seem to work the way I expect."
...
Run `pnpm create kastrax@latest` and select Cursor or Windsurf when prompted to install the MCP server. For other IDEs, or if you already have a Kastrax project, install the MCP server by following the instructions below.
...
<img
  src="/image/enable-kastrax-docs-cursor.png"
  alt="Diagram showing cursor prompt to enable Kastrax docs MCP server"
  width={800}
/>
...
3. Click "enable" on the Kastrax MCP server
...
2. If tool calls start failing, go to Windsurfs MCP settings and re-start the MCP server. This is a common Windsurf MCP issue and isn't related to Kastrax. Right now Cursor's MCP implementation is more stable than Windsurfs is.
...
Track updates for Kastrax and `@kastrax/*` packages:
